...
- **Upstream:** filed per upstream-first — see `fixes/patch012-upstream-pr.md` for the PR (branch `fix/cron-teardown-race` on the `chazmaniandinkle` fork). Record the PR number here once open. This local patch drops out at rebase when it merges.
...
| 2026-07-04 | Added | PATCH-012 | Root-cause fix for the PATCH-011 race: cron job teardown deferred to worker completion via `add_done_callback`, bounded by `HERMES_CRON_TEARDOWN_HARDCAP`; subprocess kill moved into the timeout branch. Upstream-first: PR filed before local application. |
