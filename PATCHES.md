...
- **Deferred (not applied, needs operator review):** a second, root-cause diff was proposed against `cron/scheduler.py` (skip the `_session_db.close()`/`agent.close()` teardown in the `finally:` block specifically when the inactivity-timeout branch fired and the future is not yet `.done()`) — the investigation explicitly flagged this as "a sketch, not a verified patch," noting it changes fd/subprocess-lifetime behavior (resources could be held open indefinitely if the orphaned future never completes) and recommended a `future.add_done_callback`-based refinement instead of a boolean skip, "out of scope for read-only investigation." Per this session's ground rules (do not improvise past a non-minimal/unsafe fix), that diff is NOT applied. If ratified later it would land as its own **PATCH-012** (separate file/mechanism, separately reviewable) — proposed diff preserved in `~/workspaces/cog/.cog/mem/working/2026-07-04-hermes-triage/fixes/inv-persistence.md` part (b).
