...
def test_default_zero_spawns_immediately(monkeypatch):
    ...
    # The review fork ran synchronously at turn end (stock path). Fork teardown order is
    # upstream's business (it changed at v2026.9.x), so only the spawn is asserted here.
    assert events[:2] == ["init", "run_conversation"]
...
def test_new_message_resets_debounce_then_fires_after_idle(monkeypatch):
    ...
    assert spy.calls[0]["snapshot"] == snapshot
...
def test_coalesce_one_review_covers_n_turns(monkeypatch):
    ...
    assert spy.calls[0]["snapshot"] == snap3, "newest snapshot covers all turns"
    assert spy.calls[0]["review_memory"] is True
